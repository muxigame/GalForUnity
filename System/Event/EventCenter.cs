...
namespace GalForUnity.System.Event{
	...
	public sealed class EventCenter{
        private readonly Dictionary<string, UnityAction> _events;                      //不带参委托
        private readonly Dictionary<string, UnityAction<object>> param_events;         //带参委托
        private readonly Dictionary<string, Func<object>> return_events;               //带参委托
        private readonly Dictionary<string, Func<object, object>> return_parem_events; //带参委托
        ...
        public T RequestData<T>(string _event){
            ...
            if (return_events.ContainsKey(_event)){ //如果key存在
                ...
                if (action != null) return (T) action.Invoke(); //回调
            }
        ...
        public T RequestData<T>(string _event, object param){
            ...
            if (return_parem_events.ContainsKey(_event)){ //如果key存在
                ...
                if (action != null) return (T) action.Invoke(param); //回调
            }
        ...
        public void CancelRegister(string key){ //移除监听
            if (key == null)                    //如果空键
                throw new NullReferenceException();
            if (_events.ContainsKey(key)) _events.Remove(key);
            if (param_events.ContainsKey(key)) param_events.Remove(key);
            if (return_events.ContainsKey(key)) return_events.Remove(key);
            if (return_parem_events.ContainsKey(key)) return_parem_events.Remove(key);
        }
...
#pragma warning restore 612
        ...
        /// </summary>
        /// <returns></returns>
        public static EventCenter GetInstance(){ return eventCenter; }
