...
namespace GalForUnity.Graph.GFUNode.Base{
    ...
    public class GfuNode:EditorNode{
        ...
        public GfuNode(NodeData nodeData=null){ 
        ...
        public virtual RoleData Execute(RoleData roleData){
        ...
        /// NormalMethod 判断当前节点是否还有可能存在的下一个节点
        /// </summary>
        public virtual GfuNode Next(int portIndex){
            return GfuGraph.GetNode(nodeData.OutputPort?[portIndex]?.connections?.FirstOrDefault()?.Input.instanceID??0);
        }
        ...
        public List<object> InputPortData{
            get{
                ...
                nodeData.InputPort?.ForEach((element => {
                    inputPort.Add(element);
                }));
                return inputPort;
            }
        }
