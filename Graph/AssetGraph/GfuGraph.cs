...
using GalForUnity.Graph.AssetGraph.GFUNode.Logic;
...
namespace GalForUnity.Graph.AssetGraph{
    ...
    public class GfuGraph : EditorGraph,IDisposable{
        ...
        public Action<GfuNode,GfuNode> OnNodeExecuted;
        public Action OnGraphExecuted;
        ...
        public void Stop(){
            ...
            {
                if(_currentNode is OptionNode optionNode) optionNode.HideOption();
                if (_currentNode is PlotItemNode itemNode){
                    EventCenter.GetInstance().OnMouseDown.RemoveListener(itemNode.OnMouseDown);
                }
                _currentNode.OnExecuted = null;
                _currentNode = null;
            }
            OnGraphExecuted?.Invoke();
        }
        /// <summary>
        ...
        public void Execute(){
            ...
            if (Root != null){
                _currentNode = Root;
                Root.OnExecuted += (x)=>OnNodeExecuted?.Invoke(x,GetNextNode());
        ...
        public override void Execute(GfuNode nodeObj){
            ...
            if (nodeObj != null){
                nodeObj.OnExecuted += (x)=>OnNodeExecuted?.Invoke(x,GetNextNode());
        ...
        public bool Next(int index){
            ...
            GfuRunOnMono.FixedUpdate(delegate{
                ...
                gfuNode.OnExecuted += (x)=>OnNodeExecuted?.Invoke(x,GetNextNode());
        ...
        /// <returns>是否存在下下一个节点</returns>
        /// <exception cref="NullReferenceException">未找到下一个node是触发空指针异常</exception>
        public GfuNode GetNextNode(int index=0){
            return _currentNode?.Next(index);;
        }
        /// <summary>
