...
namespace GalForUnity.Graph.AssetGraph.GFUNode.Base{
    ...
    public class GfuNode : EditorNode,ISerializable{
        ...
        [NonSerialized]
        private INodeRuntimeProviderBase _nodeFindProvider;
        [NonSerialized]
        public long instanceID;
        [NonSerialized]
        private GfuNodeAsset _gfuNodeAsset;
        public GfuNodeAsset gfuNodeAsset=>_gfuNodeAsset;
        ...
        public void InitWithGfuNodeData(GfuNodeAsset gfuNodeAsset,GfuNodeData gfuNodeData,INodeRuntimeProviderBase nodeFindProvider){
            _gfuNodeAsset = gfuNodeAsset;
            if (gfuNodeData == null){
                Debug.LogError("gfuNodeData is null cannot init node");
                Init(null);
                return;
            }
            instanceID = gfuNodeData.instanceID;
            _nodeFindProvider = nodeFindProvider;
            var type = GetType();
    
            foreach (var fieldData in gfuNodeData.objects){
                SetValue(fieldData);
            }
            foreach (var fieldData in gfuNodeData.values){
                SetValue(fieldData);
            }
            Init(null);
            foreach (var fieldData in gfuNodeData.objects){
                SetPortValue(fieldData);
            }
            foreach (var fieldData in gfuNodeData.values){
                SetPortValue(fieldData);
            }
        ...
        public void SetValue<T>(GfuNodeData.FieldData<T> fieldData){
            if (fieldData.fieldName.StartsWith(GalGraph.PortViewName)) return;
            if(fieldData.FieldValue == null||string.IsNullOrWhiteSpace(fieldData.FieldValue.ToString())) return;
            var fieldInfo = GetType().GetField(fieldData.fieldName);
            if (fieldInfo.FieldType == typeof(string)){
                fieldInfo.SetValue(this,fieldData.FieldValue);
            } else if (fieldInfo.FieldType.IsPrimitive){
                fieldInfo.SetValue(this,Convert.ChangeType(fieldData.FieldValue,fieldInfo.FieldType));
            }else if (fieldData.FieldValue is Object obj){
                if(obj) fieldInfo.SetValue(this,obj);
            } else{
                fieldInfo.SetValue(this,JsonUtility.FromJson(fieldData.FieldValue.ToString(),fieldInfo.FieldType));
            }
        }  
        public void SetPortValue<T>(GfuNodeData.FieldData<T> fieldData){
            if(fieldData.FieldValue == null ||string.IsNullOrWhiteSpace(fieldData.FieldValue.ToString())) return;
            if (fieldData.fieldName.StartsWith(GalGraph.PortViewName)){
                int index = int.Parse(fieldData.fieldName.Replace(GalGraph.PortViewName, "").Trim());
                var gfuOperationNode = (this as GfuOperationNode);
                // gfuOperationNode?.InitDefaultValuePort(GetType(),null);
                var gfuInputView = gfuOperationNode.GfuInputViews[index];
                var fieldType = gfuInputView.portType;
                if (fieldType == typeof(string)){
                    gfuInputView.Value=fieldData.FieldValue;
                } else if (fieldType.IsPrimitive){
                    gfuInputView.Value=Convert.ChangeType(fieldData.FieldValue,fieldType);
                }else if (fieldData.FieldValue is Object obj){
                    gfuInputView.Value=obj;
                } else{
                    gfuInputView.Value=JsonUtility.FromJson(fieldData.FieldValue.ToString(),fieldType);
                }
                return;
            }
        }
        ...
        public virtual void Executed(int index){
            if (_nodeFindProvider != null){
                OnExecuted?.Invoke(_nodeFindProvider.GetOutputNode(index,0));
                return;
            }
        ...
        public int GetOutputConnectionCount(int index=0){
            return _nodeFindProvider.GetOutputPortConnectionCount(index);
        }      
        public int GetInputConnectionCount(int index=0){
            return _nodeFindProvider.GetInputPortConnectionCount(index);
        }
        ...
        public int InputPortCount{
            get{
                if (_nodeFindProvider != null) return _nodeFindProvider.InputPortCount(); 
        ...
        public int OutputPortCount{
            get{
                if (_nodeFindProvider != null) return _nodeFindProvider.OutputPortCount(); 
