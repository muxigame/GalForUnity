...
namespace GalForUnity.Graph.AssetGraph.GFUNode.Operation{
    ...
    /// 链接的默认值
    /// </summary>
    [NodeType(NodeCode.GfuOperationNode)]
    public class GfuOperationNode : GfuNode{
        ...
        public override void Init(NodeData otherNodeData){
            ...
            if(GfuInputViews ==null)GfuInputViews = new List<GfuInputView>();
#if UNITY_EDITOR
        ...
        public void InitDefaultValuePort(Type type,NodeData nodeData){
            if(GfuInputViews ==null)GfuInputViews = new List<GfuInputView>();
            Type[] types = new Type[InputPortCount];
            for (var i = 0; i < types.Length; i++){
                if (nodeData &&nodeData.InputPort.Count ==InputPortCount) types[i] = nodeData.InputPortType(i);
            ...
            GfuOperation = (GfuOperation) Activator.CreateInstance(type);
            GfuOperation.Input = GfuOperationData.CreateInstance(types);
            GfuOperation.Container = GfuOperationData.CreateInstance(ContainerType());
        ...
        public List<AssetGraph.Operation.Data> GetDefaultValue(){
            List<AssetGraph.Operation.Data> datas = new List<AssetGraph.Operation.Data>();
            for (int i = 0; i < GfuInputViews.Count; i++){
                datas.Add(new AssetGraph.Operation.Data(GetDefaultValue(i)));
            }
            return datas;
        }
